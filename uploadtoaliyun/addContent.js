...
var uploader = new plupload.Uploader({
    ...
    init: {
        ...
        UploadProgress: function(up, file) {
            ...
            var addVideoTips = file.percent + " % 100"
            $("#addVideoTips").text(addVideoTips) ;
        },

        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                videoInfo.videoId = get_uploaded_object_name(file.name) ;
                console.log(videoInfo.videoId);

                var addVideoTips = "视频上传成功!!"
        ...
        Error: function(up, err) {
            ...
            var addVideoTips = "<b style='color:red'>上传出错!!</b>" ;
            $("#addVideoTips").text(addVideoTips) ;
        }
    }
});
